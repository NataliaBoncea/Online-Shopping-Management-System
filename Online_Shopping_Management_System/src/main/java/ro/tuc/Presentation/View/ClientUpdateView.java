...
public class ClientUpdateView extends JFrame{
    ...
    public ClientUpdateView(ClientView clientView){
        ...
        idT.setText(Integer.toString(clientBLL.findClientById(clientView.getSelectedId()).getId()));

        p2.add(numeL);
        p2.add(numeT);
        p2.setBackground(purple2);
        p2.setLayout(new FlowLayout());
        numeT.setText(clientBLL.findClientById(clientView.getSelectedId()).getNume());

        p3.add(prenumeL);
        p3.add(prenumeT);
        p3.setBackground(purple2);
        p3.setLayout(new FlowLayout());
        prenumeT.setText(clientBLL.findClientById(clientView.getSelectedId()).getPrenume());

        p4.add(adresaL);
        p4.add(adresaT);
        p4.setBackground(purple2);
        p4.setLayout(new FlowLayout());
        adresaT.setText(clientBLL.findClientById(clientView.getSelectedId()).getAdresa());

        p5.add(emailL);
        p5.add(emailT);
        p5.setBackground(purple2);
        p5.setLayout(new FlowLayout());
        emailT.setText(clientBLL.findClientById(clientView.getSelectedId()).getEmail());

        p6.add(telefonL);
        p6.add(telefonT);
        p6.setBackground(purple2);
        p6.setLayout(new FlowLayout());
        telefonT.setText(clientBLL.findClientById(clientView.getSelectedId()).getNr_telefon());
